...
static void enter_state(MenuState_t st)
{
    ...
    /* Немедленная перерисовка экрана при любом переходе состояния.
       Без этого смена экрана (POWER_ON→MAIN, MAIN→STANDBY, вход/выход
       редактора и т. д.) была бы видна только на следующем «тике»
       DISP_TICKS (до 1.5 с), что воспринималось как «надписи не
       переключаются».                                                    */
    s_disp_tick = 0u;
    menu_update_display();
}
...
void menu_process(BtnEvent_t ev)
{
    ...
    /* Отрисовка экрана — в конце, чтобы увидеть результат обработки
       кнопки в ЭТОМ же кадре (а не через 1.5 с — до того курсор «скакал»
